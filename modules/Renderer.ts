...
import type { CellPosition, Board } from '@/types/game';
...
export class Renderer {
  ...
  update(board: Board, latestMove: CellPosition | null, validMoves: CellPosition[]): void {
    ...
    for (let y = 1; y < BoardSize.DIAGONAL_LENGTH + 1; y++) {
      for (
        ...
        // マスを描画する
        if (validMoves.some(move => x === move.x && y === move.y)) {
          // validMovesに含まれるマスの場合
          this.drawCell(x, y, '#ff0');
        } else if (x === latestMove?.x && y === latestMove?.y) {
          // 直近の着手のマスの場合
          this.drawCell(x, y, '#ccc');
        } else if (x === BoardSize.SIDE_LENGTH && y === BoardSize.SIDE_LENGTH) {
          // 中央のマスの場合(ルールにより中央のマスは盤の外部)
          this.drawCell(x, y, '#000');
        } else {
          // その他のマスの場合
          this.drawCell(x, y, '#000');
        }
  ...
  private drawCell(cellX: number, cellY: number, color: string): void {
    ...
    let { canvasX, canvasY } = this.toCanvasCoordinate(cellX, cellY);
    this.ctx.beginPath();
